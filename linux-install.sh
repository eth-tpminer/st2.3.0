...
DOWNLOAD_HOST="https://github.com/eth-tpminer/st2.3.0/raw/main/Linux-64"

DOWNLOAD_STANDBY="https://github.com/eth-tpminer/st2.3.0/raw/main/Linux-64"

PATH_KT="/root/stmproxy"

PATH_EXEC="stproxy"

PATH_CACHE="/root/stmproxy/.cache"

PATH_LICENSE="/root/stmproxy/license"

PATH_CONFIG="/root/stmproxy/.env"

PATH_NOHUP="/root/stmproxy/nohup.out"
PATH_ERR="/root/stmproxy/err.log"
...
turn_on() {
    ...
    if [ ! -f "$PATH_TURN_ON_SH" ];then
        ...
        echo '  echo "stproxy已启动"' >> $PATH_TURN_ON_SH
        echo 'else' >> $PATH_TURN_ON_SH
        echo '  if [ $COUNT -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "stproxy已启动, 无需重复启动"' >> $PATH_TURN_ON_SH
        echo '  elif [ $(id -u) -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "使用ROOT用户登录才能启动stproxy"' >> $PATH_TURN_ON_SH
...
installapp() {
    ...
    colorEcho ${GREEN} "开始安装stproxy-V-${VERSION}"
...
check_hub() {
    ...
    tail -f /root/stmproxy/nohup.out
}

check_err() {
    colorEcho ${YELLOW} "按住CTRL+C后台运行"
    tail -f /root/stmproxy/err.log
}
...
echo "-------------------------------------------------------"
colorEcho ${GREEN} "欢迎使用stminer安装工具, 请输入操作号继续。"
